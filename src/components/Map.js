...
// Komponen perbaikan untuk masalah ukuran/overlap
const MapResizer = () => {
  const map = useMap();
  useEffect(() => {
  ...
  return (
    <MapContainer center={[-2.5, 118]} zoom={4.5} scrollWheelZoom={true} style={{ height: '100%', width: '100%', borderRadius: '0.75rem', backgroundColor: 'transparent' }}>
      ...
      {indonesiaGeoJson.features.map(provinsi => {
        ...
        return (
          <CircleMarker key={provinsi.properties.name} center={[provinsi.geometry.coordinates[1], provinsi.geometry.coordinates[0]]} pathOptions={{ color: 'white', weight: 1, fillColor: getColor(count), fillOpacity: 0.7 }} radius={getRadius(count)}>
