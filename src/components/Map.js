...
// --- KOMPONEN BARU UNTUK MEMPERBAIKI UKURAN PETA ---
const MapResizer = () => {
  const map = useMap();
  useEffect(() => {
    setTimeout(() => {
      map.invalidateSize();
    }, 100); // Diberi jeda sedikit untuk memastikan container sudah dirender
  }, [map]);
  return null;
};
...
const Map = ({ mapData }) => {
  ...
  return (
    <MapContainer center={[-2.5, 118]} zoom={4.5} scrollWheelZoom={true} style={{ height: '100%', width: '100%', borderRadius: '0.75rem', backgroundColor: 'transparent' }}>
      ...
      <MapResizer /> {/* <-- KOMPONEN PERBAIKAN DITAMBAHKAN DI SINI */}
    </MapContainer>
  );
};
