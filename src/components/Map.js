...
// ... (Salin kembali objek indonesiaGeoJson, getColor, dan getRadius Anda ke sini) ...
...
const Map = ({ mapData }) => {
  const mapContainerRef = useRef(null); // Ref untuk menunjuk ke div container

  useEffect(() => {
    // Log ini akan berjalan setelah komponen dirender
    if (mapContainerRef.current) {
      const { clientWidth, clientHeight } = mapContainerRef.current;
      console.log(`DIAGNOSA PETA: Ukuran container saat render: ${clientWidth}px x ${clientHeight}px`);
    }
  }, []);
  ...
  return (
    // Beri ref ke div container peta
