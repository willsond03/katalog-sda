  ...
  return (
    <>
      <div className="p-4 md:p-8 space-y-8">
        <header>
          <h1 className="text-3xl font-bold text-gray-800">Market Sounding</h1>
        </header>
        ...
        <div className="p-6 bg-white border border-gray-200 rounded-xl shadow-sm">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Analisis Market Sounding</h3>
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <label htmlFor="comparisonDate" className="block text-sm font-medium text-gray-700">Pilih Event</label>
              <select id="comparisonDate" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm h-10 px-3 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div className="flex-1">
              ...
              <input type="number" id="comparisonDay" defaultValue="7" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm h-10 px-3 focus:border-blue-500 focus:ring-blue-500"/>
            </div>
        ...
        {loadingAnalysis && <div className="p-6 bg-white border border-gray-200 rounded-xl shadow-sm text-center">Menganalisa data...</div>}
        {analysisResult && (
          <div className="p-6 bg-white border border-gray-200 rounded-xl shadow-sm">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Hasil Analisa</h3>
      ...
      {modals.marketSounding && ( <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50"><div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md"><h2 className="text-xl font-bold mb-4 text-gray-800">Catat Market Sounding</h2><form onSubmit={handleMarketSoundingSubmit} className="space-y-4"><div><label htmlFor="balai" className="block text-sm text-gray-700">Balai</label><input type="text" id="balai" name="balai" required className="mt-1 block w-full rounded-md border-gray-300 h-10 px-3"/></div><div><label htmlFor="provinsi-modal" className="block text-sm text-gray-700">Wilayah</label><select id="provinsi-modal" name="provinsi" required className="mt-1 block w-full rounded-md border-gray-300 h-10 px-3">{provinsiOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div><div><label htmlFor="paket_pekerjaan" className="block text-sm text-gray-700">Paket Pekerjaan</label><input type="text" id="paket_pekerjaan" name="paket_pekerjaan" required className="mt-1 block w-full rounded-md border-gray-300 h-10 px-3"/></div><div><label htmlFor="tanggal" className="block text-sm text-gray-700">Tanggal</label><input type="date" id="tanggal" name="tanggal" required className="mt-1 block w-full rounded-md border-gray-300 h-10 px-3"/></div><div className="flex justify-end space-x-4 pt-4"><button type="button" onClick={() => closeModal('marketSounding')} className="py-2 px-4 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg">Batal</button><button type="submit" className="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Simpan</button></div></form></div></div> )}
      {modals.history && ( <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50"><div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-5xl"><div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-gray-800">Histori Market Sounding</h2><button onClick={() => closeModal('history')} className="text-gray-500 text-2xl font-bold">&times;</button></div><div className="overflow-y-auto max-h-[70vh]">{loadingHistory ? <p className="text-center py-10">Memuat histori...</p> : (historyData && historyData.length > 0 ? (<table className="min-w-full"><thead className="bg-gray-100 sticky top-0"><tr><th className="px-6 py-3 text-left text-xs font-semibold uppercase">Tanggal</th><th className="px-6 py-3 text-left text-xs font-semibold uppercase">Balai</th><th className="px-6 py-3 text-left text-xs font-semibold uppercase">Wilayah</th><th className="px-6 py-3 text-left text-xs font-semibold uppercase">Paket Pekerjaan</th></tr></thead><tbody className="divide-y divide-gray-200">{historyData.map((log) => (<tr key={log.id}><td className="px-6 py-4 text-sm whitespace-nowrap">{log.tanggal}</td><td className="px-6 py-4 text-sm whitespace-nowrap">{log.balai}</td><td className="px-6 py-4 text-sm whitespace-nowrap">{log.wilayah}</td><td className="px-6 py-4 text-sm">{log.paket_pekerjaan}</td></tr>))}</tbody></table>) : (<p className="text-center text-gray-500 py-10">Tidak ada data histori.</p>))}</div></div></div> )}
    </>
  );
}